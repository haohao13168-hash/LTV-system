...
// One-time migration: previous default brand names → the new one.
// We migrate exact matches only so a user's custom brand name is preserved.
const BRAND_MIGRATIONS = new Set([
  "Marketing Console",
  "Marketing Dashboard",
  "Parthner Shift",
]);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        // Migrate known old brand defaults to the new default. A custom
        // brand the user set themselves stays untouched.
        if (parsed.brandName && BRAND_MIGRATIONS.has(parsed.brandName)) {
          parsed.brandName = defaultSettings.brandName;
          parsed.brandInitial = defaultSettings.brandInitial;
        }
