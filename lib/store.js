...
export function StoreProvider({ children }) {
  ...
  // Returns { ok, from, to, platforms: [...], total } — does NOT write to DB,
  // each call is fresh from the wallet API. Takes ~30-60s.
  const fetchBcbRange = useCallback(async (from, to) => {
    try {
      const res = await fetch("/api/bcb/range", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ from, to }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) return { error: data?.error || `Range query failed (${res.status})` };
      return data;
    } catch (e) {
