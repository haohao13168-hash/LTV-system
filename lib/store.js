...
export function StoreProvider({ children }) {
  ...
  // Uses async polling so Vercel's 10s function timeout doesn't kill it.
  const syncBcbNow = useCallback(async () => {
    if (bcbSyncing) return { error: "Sync already in progress" };
    setBcbSyncing(true);
    try {
      // 1. Start the sync job
      const startRes = await fetch("/api/bcb/sync/start", { method: "POST" });
      const startData = await startRes.json().catch(() => ({}));
      if (!startRes.ok || !startData?.jobId) {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        if (!sRes.ok) {
          return { error: sData?.error || `Status check failed (${sRes.status})` };
        }
        ...
        if (sData.status === "error") {
          return { error: sData.error || "Sync failed" };
        }
      }
