...
// Parse wallet name out of a wallet_source value: 'BCB_TOTAL' → 'BCB',
// 'V12MY_TOTAL' → 'V12MY'. Returns null if no wallet linked.
...
// Stats for a wallet-linked company. Sums only platforms belonging to that
// wallet (so BCB company doesn't accidentally pick up V12MY rows).
function statsFromBcb(walletSource, bcbPlatforms) {
  const walletId = walletIdFromSource(walletSource);
  if (!walletId) return { members: 0, deposit: 0, withdraw: 0, net: 0 };
  const rows = bcbPlatforms.filter((p) => (p.wallet || "BCB") === walletId);
  const out = { members: 0, deposit: 0, withdraw: 0, net: 0 };
  for (const p of rows) {
...
// Latest sync timestamp for one wallet's platforms (or globally if not given)
function latestSyncedAt(bcbPlatforms, walletId = null) {
  let latest = null;
  for (const p of bcbPlatforms) {
    if (walletId && (p.wallet || "BCB") !== walletId) continue;
...
export function StoreProvider({ children }) {
  ...
  const fetchBcbRange = useCallback(async (walletId, from, to, onProgress) => {
    if (!walletId) return { error: "fetchBcbRange needs a walletId" };
    try {
      // 1. Kick off the job
      const startRes = await fetch("/api/bcb/range/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ wallet: walletId, from, to }),
      });
      ...
      // 2. Poll status — start fast (snapshot hits are sub-second), back off
      // to 3s for slow live-API fallbacks. Sequence: 250ms · 500ms · 1s ·
      // 2s · 3s · 3s · 3s … so snapshot-fast queries return in <1s instead
      // of waiting a flat 3s minimum.
      const delays = [250, 500, 1000, 2000];
      const startedAt = Date.now();
      const MAX_TOTAL_MS = 10 * 60 * 1000; // 10 min cap
      let attempt = 0;
  ...
  // Trigger droplet to pull fresh wallet data, then refresh from Supabase.
  // Takes walletId so we can refresh just BCB, just V12MY, etc.
  const syncBcbNow = useCallback(async (walletId) => {
    if (!walletId) return { error: "syncBcbNow needs a walletId" };
    if (bcbSyncing) return { error: "Sync already in progress" };
    setBcbSyncing(true);
    try {
      const startRes = await fetch("/api/bcb/sync/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const startData = await startRes.json().catch(() => ({}));
      if (!startRes.ok || !startData?.jobId) {
        return { error: startData?.error || `Failed to start sync (${startRes.status})` };
      }
      const jobId = startData.jobId;
      for (let i = 0; i < 200; i++) {
        await new Promise((r) => setTimeout(r, 3000));
        const sRes = await fetch(`/api/bcb/range/status?id=${encodeURIComponent(jobId)}`);
        const sData = await sRes.json().catch(() => ({}));
        if (!sRes.ok) return { error: sData?.error || `Status check failed (${sRes.status})` };
        if (sData.status === "done") {
          await refresh();
          return { ok: true, ...sData };
        }
        if (sData.status === "error") return { error: sData.error || "Sync failed" };
      }
      return { error: "Sync timed out (>10 min)" };
    } catch (e) {
      return { error: e.message || String(e) };
    } finally {
      setBcbSyncing(false);
  ...
  // Stats aggregated from OTHER companies (excluding this one).
  // We deliberately skip every wallet-aggregator (_TOTAL) so a company
  // bound to its own wallet doesn't get inflated by BCB's or V12MY's
  // totals. Platform-level placeholders (no _TOTAL) also contribute 0.
  const getReceivedStats = (company, range = {}) => {
    if (!company) return { members: 0, deposit: 0, withdraw: 0, net: 0 };
    const out = { members: 0, deposit: 0, withdraw: 0, net: 0 };
    for (const c of companies) {
      if (c.id === company.id) continue;
      if (c.walletSource && c.walletSource.endsWith("_TOTAL")) continue;
